...
fn get_signal(this: &JsValue, _: &[JsValue], _: &mut Context) -> JsResult<JsValue> {
    ...
    with_abort_controller_ref(&controller, |controller| {
        JsValue::from(controller.signal_value())
    })
}
...
fn abort(this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    let signal = with_abort_controller_ref(&controller, |controller| controller.signal_value())?;
