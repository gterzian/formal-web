...
fn with_document<R>(this: &JsValue, f: impl FnOnce(&Document) -> R) -> JsResult<R> {
    let object = this.as_object().ok_or_else(|| {
        JsNativeError::typ().with_message("document receiver is not an object")
    })?;
    let document = object.downcast_ref::<Document>().ok_or_else(|| {
        JsNativeError::typ().with_message("receiver is not a Document")
    })?;
    Ok(f(&document))
}
...
fn query_selector(this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    let selector = args.get_or_undefined(0).to_string(context)?.to_std_string_escaped();
...
fn create_element(this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    let local_name = args.get_or_undefined(0).to_string(context)?.to_std_string_escaped();
...
fn create_text_node(this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    let (document, node_id) = with_document(this, |document| {
        (std::rc::Rc::clone(&document.node.document), document.create_text_node(&text))
    })?;
...
fn get_title(this: &JsValue, _: &[JsValue], _: &mut Context) -> JsResult<JsValue> {
    with_document(this, |document| JsValue::from(JsString::from(document.title().as_str())))
}
