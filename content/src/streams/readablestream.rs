...
fn readable_stream_default_tee(
    ...
    // Step 19: "Upon rejection of reader.[[closedPromise]] with reason r,"
    let on_rejected =
        ec.create_builtin_function_with_captures(tee_state, default_tee_on_rejected_fn, 1, ec.property_key_from_str(""));
...
impl PipeToState {
    ...
    fn write_chunk(
        ...
        self.0.borrow_mut().pending_writes.push_back(write_promise);
        Ok(true)
    }
    ...
    fn perform_action(
        ...
        self.0.borrow_mut().shutdown_action_promise = Some(action_promise.clone());
    ...
    fn pending_write_front(&self) -> Option<JsObject> {
        self.0.borrow().pending_writes.front().cloned()
    }
...
fn start_abort_cancel_source(
    ...
    let on_fulfilled =
        ec.create_builtin_function_with_captures(state.clone(), start_abort_cancel_on_fulfilled_fn, 0, ec.property_key_from_str(""));
    let on_rejected =
        ec.create_builtin_function_with_captures(state, start_abort_cancel_on_rejected_fn, 1, ec.property_key_from_str(""));
