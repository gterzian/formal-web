...
| Blank line missing between `// Step N:` comment and following Rust code | Always insert a blank line between the last `// Step N:` comment and the next line of Rust code. This makes step boundaries visually distinct. See `dispatch.rs` for the correct pattern. |
