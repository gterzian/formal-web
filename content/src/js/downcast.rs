...
//!
//! These call `JsObject::downcast_ref` / `downcast_mut`, which only exists
//! on `boa_engine::object::JsObject`.  The generic alternatives live in
//! `downcast_generic.rs` and use `ExecutionContext::with_object_any`.
