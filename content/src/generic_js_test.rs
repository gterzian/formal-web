...
mod tests {
    ...
    fn gc_root_survives_throwaway_pressure() {
        ...
        // GcRootHandle; on JSC the managed reference prevents collection.
        drop(root);
    }

    /// Verifies the engine's `gc()` actually reclaims unreferenced objects
    /// (detected via `FinalizationRegistry`).  This is the precondition for
    /// the whole rooting design: only if the GC collects do released roots
    /// matter.
    ///
    /// All backends: on Boa `gc()` is a real collection; on JSC it is
    /// `JSGarbageCollect`.
    #[test]
    fn gc_reclaims_unreferenced_objects() {
        ...
        // Register a throwaway object in a FinalizationRegistry and drop
        // all JS references to it.
        JsEngine::evaluate_script(
            &mut engine,
            // Allocation pressure: JSC only reclaims when the heap needs
            // a collection; a few rounds of garbage ensure gc() collects.
            "var collected = false;\
             var registry = new FinalizationRegistry(function() { collected = true; });\
             var target = {};\
             registry.register(target);\
             target = null;\
             var garbage = [];\
             for (var i = 0; i < 2000; i++) garbage.push(new Array(1000).fill(i));\
             garbage = null;",
            &realm,
        )
        .unwrap();

        // Force GC a few times and run the cleanup jobs.
        for _ in 0..3 {
            engine.gc();
            let _ = JsEngine::evaluate_script(&mut engine, "void 0;", &realm);
        }
        engine.perform_a_microtask_checkpoint();

        let result = JsEngine::evaluate_script(&mut engine, "collected", &realm).unwrap();
        assert!(
            engine.to_boolean(&result),
            "unreferenced object should be collected after gc()"
        );
    }

    /// A value rooted via `create_root` must survive GC while the handle
    /// lives — the protection half of the managed-reference contract.
    ///
    /// The release half (collectable once the handle is dropped) is not
    /// deterministically observable from Rust tests: on JSC the raw
    /// `JscValue` locals in the test frame are conservatively stack-scanned
    /// and keep the value alive, and on Boa GC-while-held re-roots the
    /// object (see `js_engine/README.md`).  The release is verified by the
    /// ObjC `addManagedReference`/`removeManagedReference` mechanism
    /// directly (see the session log in `js_engine/README.md`).
    #[test]
    fn root_keeps_value_alive_until_dropped() {
        let mut engine = setup();
        let realm = engine.current_realm();

        // Create an object in JS and register it in a FinalizationRegistry.
        JsEngine::evaluate_script(
            &mut engine,
            "var rootTarget = {};\
             var collected = false;\
             var registry = new FinalizationRegistry(function() { collected = true; });\
             registry.register(rootTarget);\
             var garbage = [];\
             for (var i = 0; i < 2000; i++) garbage.push(new Array(1000).fill(i));\
             garbage = null;",
            &realm,
        ...
        .unwrap();
        let obj_val = JsEngine::evaluate_script(&mut engine, "rootTarget", &realm).unwrap();

        // Root it from Rust and drop the JS-side reference.
        let root = engine.create_root(&obj_val);
        JsEngine::evaluate_script(&mut engine, "rootTarget = null;", &realm).unwrap();

        // GC pressure while rooted: the value must survive.
        for _ in 0..3 {
            engine.gc();
        }
        engine.perform_a_microtask_checkpoint();
        let survived = JsEngine::evaluate_script(&mut engine, "collected", &realm).unwrap();
        assert!(
            !engine.to_boolean(&survived),
            "rooted value must survive gc() while the handle lives"
        );

        // Dropping the handle must not corrupt anything; the value may or
        // may not be reclaimed depending on the platform GC (see the
        // doc comment above).
        drop(root);
    }

    /// `JsValueCell` keeps a JS value alive while the cell (or any clone)
    /// lives — the protection half of the managed-value cell contract.
    ///
    /// The release half (collectable once the last clone drops) is not
    /// deterministically observable from Rust tests: on JSC the raw
    /// `JscValue` locals in the test frame are conservatively stack-scanned
    /// and keep the value alive, and on Boa GC-while-held re-roots the
    /// object (see `js_engine/README.md`).
    #[test]
    fn js_value_cell_keeps_value_alive_then_releases() {
        use js_engine::gc::JsValueCell;

        let mut engine = setup();
        let realm = engine.current_realm();

        // Create an object in JS, register it, and hand it to a cell.
        JsEngine::evaluate_script(
            &mut engine,
            "var cellTarget = {};\
             var collected = false;\
             var registry = new FinalizationRegistry(function() { collected = true; });\
             registry.register(cellTarget);\
             var garbage = [];\
             for (var i = 0; i < 2000; i++) garbage.push(new Array(1000).fill(i));\
             garbage = null;",
            &realm,
        ...
        .unwrap();
        let obj_val = JsEngine::evaluate_script(&mut engine, "cellTarget", &realm).unwrap();
        JsEngine::evaluate_script(&mut engine, "cellTarget = null;", &realm).unwrap();

        let cell = JsValueCell::new(obj_val);
        let cell_clone = cell.clone();

        // GC pressure while the cell lives: the value must survive.
        for _ in 0..3 {
            engine.gc();
        }
        engine.perform_a_microtask_checkpoint();
        ...
        assert!(
            !engine.to_boolean(&survived),
            "value must survive gc() while the cell lives"
        );

        // Drop one clone — the value must still survive (other clone holds it).
        drop(cell_clone);
        for _ in 0..3 {
            engine.gc();
        }
        ...
        assert!(
            !engine.to_boolean(&survived),
            "value must survive while any cell clone lives"
        );

        // Dropping the last clone must not corrupt anything; the value may
        // or may not be reclaimed depending on the platform GC (see the
        // doc comment above).
        drop(cell);
    }

    /// `JsObjectCell` keeps an optional JS object alive while the cell
    /// lives — the protection half of the managed-object cell contract.
    ///
    /// The release half (collectable once cleared/dropped) is not
    /// deterministically observable from Rust tests: on JSC the raw
    /// `JscValue` locals in the test frame are conservatively stack-scanned
    /// and keep the value alive, and on Boa GC-while-held re-roots the
    /// object (see `js_engine/README.md`).
    #[test]
    fn js_object_cell_keeps_value_alive_then_releases() {
        ...
        JsEngine::evaluate_script(
            ...
            "var cellObjTarget = {};\
             var collected = false;\
             var registry = new FinalizationRegistry(function() { collected = true; });\
             registry.register(cellObjTarget);\
             var garbage = [];\
             for (var i = 0; i < 2000; i++) garbage.push(new Array(1000).fill(i));\
             garbage = null;",
            &realm,
        ...
        .unwrap();
        let obj_val = JsEngine::evaluate_script(&mut engine, "cellObjTarget", &realm).unwrap();
        let obj = TestTypes::value_as_object(&obj_val).unwrap();
        JsEngine::evaluate_script(&mut engine, "cellObjTarget = null;", &realm).unwrap();

        let cell = JsObjectCell::new(Some(obj));
        ...
        assert!(
            !engine.to_boolean(&survived),
            "object must survive gc() while the cell lives"
        );

        // Clearing the cell must not corrupt anything; the object may or
        // may not be reclaimed depending on the platform GC (see the
        // doc comment above).
        cell.set(None);
    }
