...
#[cfg(test)]
mod tests {
    use super::{PendingParserScript, execute_parser_scripts, parse_html_into_document};
    use crate::html::EnvironmentSettingsObject;
    ...
    #[test]
    fn parse_html_into_document_skips_non_classic_script_types() {
        ...
        let scripts = {
            ...
            parse_html_into_document(
                ...
                <script type="module">globalThis.moduleLoaded = true;</script>
                <script>globalThis.inlineClassic = true;</script>
                <script type="text/javascript">globalThis.typedClassic = true;</script>
                "#,
            )
        };
        ...
        assert!(matches!(
            &scripts[1],
            PendingParserScript::Inline { source } if source.contains("typedClassic")
        ));
    }

    #[test]
    fn execute_parser_scripts_keeps_loading_after_script_error() {
        let document = Rc::new(RefCell::new(BaseDocument::new(DocumentConfig::default())));
        ...
        let result = execute_parser_scripts(
            &mut settings,
            vec![
                PendingParserScript::Inline {
                    source: String::from("not valid javascript: {"),
                },
                PendingParserScript::Inline {
                    source: String::from("globalThis.afterError = 1;"),
                },
            ],
        );
        ...
        let after_error = settings
            .context
            .global_object()
            .get(boa_engine::js_string!("afterError"), &mut settings.context)
            .expect("follow-up script should be readable");
        assert_eq!(after_error.as_number(), Some(1.0));
    }
}
