...
fn window_proxy_define_own_property(
    ...
    // Note: We need to call the low-level __define_own_property__ on the
    // Window.  There is no public wrapper that takes a PropertyDescriptor
    // directly (define_property_or_throw wraps in TypeError).  Shape the
    // descriptor through the Window's vtable via __define_own_property__.
    //
    // We use the same InternalMethodPropertyContext that was passed to us,
    // but the Window's ordinary implementation uses its own slot().  The
    // context.slot() is intended for caching — reusing it across objects
    // is safe since slot is just a cache that will be re-initialised.
...
fn window_proxy_set(
    ...
    //
    // We can use the public `set()` method which calls through the vtable
    // with `self` (the Window) as the receiver.  However, the spec says
    // to pass the original Receiver through.  The ordinary __set__
    // vtable entry handles this correctly — it uses Receiver in
    // OrdinarySetWithOwnDescriptor.  Call it directly through the vtable.
