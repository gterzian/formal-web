...
- `agent.rs` defines the agent records.  `SimilarOriginWindowAgent` is the window agent of one agent cluster, keyed by agent id in `UserAgentState::agents`; it owns its window event loop and the traversables whose documents run on it.  `DedicatedWorkerAgent` is the record of one worker agent, keyed by worker id in `UserAgentState::worker_agents` — flat, not nested under the window agent of its cluster, since a worker agent is created and destroyed while its host window agent lives on; it carries its own worker event loop and the id of its hosting window agent (so the record is dropped when the content process exits).  `AgentCluster`/`AgentClusterKey` are the per-group agent cluster records of the "agent cluster formalism" section; the UA-side records of the event loops the agents own live in `event_loops.rs`.
