...
- `agent.rs` defines the agent records.  `Agent` is the union of the two agent kinds recorded in `UserAgentState::agents` — `SimilarOriginWindowAgent` (the window agent of one agent cluster, owning its window event loop and the traversables whose documents run on it) and `DedicatedWorkerAgent` (the record of one worker agent) — registered side by side, each keyed by the event loop it owns: its window event loop for a window agent, its own worker event loop for a worker agent, the id by which content addresses the agent.  Worker agents are flat, created and destroyed while their host window agent lives on; a worker record carries its own worker event loop, the worker id it is reported and closed under, and the signifier of its hosting window agent (so the record is dropped when the content process exits).  `AgentCluster`/`AgentClusterKey` are the per-group agent cluster records of the "agent cluster formalism" section; the UA-side records of the event loops the agents own live in `event_loops.rs`.
- `event_loops.rs` defines the user-agent-side handles of the two event-loop kinds.  `WindowEventLoop` is the window event loop of a similar-origin window agent — the record of the window agent's content process (its agent cluster), whose main thread doubles as the cluster's command endpoint and as the window event loop: the command channel carries both cluster-level commands (bootstrap, viewport, document lifecycle, shutdown) and the window event loop's queued tasks, the event receiver carries the events of every agent of the cluster (worker agents report over it too), and the record holds the child process plus the script/click waiters keyed by request id.  `WorkerEventLoop` is the event loop of a dedicated worker agent: its own event loop id and its own user-agent command channel, so commands addressed to a worker event loop — the port tasks of `Command::PortTask` — go straight to the worker.  `spawn_window_event_loop` launches the content process that is the new agent's cluster and bootstraps the agent's window event loop inside it; it is called from `obtain_similar_origin_window_agent` (`UserAgentWorker`, the `#obtain-similar-origin-window-agent` algorithm), which creates the cluster and the window agent when the browsing context group's agent cluster map has no cluster for the origin's site yet, and otherwise returns the window agent of the group's existing cluster.
