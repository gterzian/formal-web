...
fn network_token_from_args() -> Result<Option<String>, String> {
    let mut args = env::args().skip(1);
    while let Some(arg) = args.next() {
        if arg == "--network-token" {
            return args
                .next()
                .map(Some)
                .ok_or_else(|| String::from("missing network token value"));
        }
    }
    Ok(None)
}
...
fn run_network_process(token: String) -> Result<(), String> {
...
pub(crate) fn maybe_run_network_process() -> Option<Result<(), String>> {
    match network_token_from_args() {
        Ok(Some(token)) => Some(run_network_process(token)),
        Ok(None) => None,
