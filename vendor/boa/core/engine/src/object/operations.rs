...
impl JsObject {
    ...
    /// Unlike `has_own_property` this returns the full `PropertyDescriptor`.
    ...
    /// [[Get]] with a custom receiver.
    ///
    /// Unlike `get()` which always passes `self` as the receiver, this
    /// propagates the original receiver through the prototype chain.
    ...
    /// [[Set]] with a custom receiver.
    ///
    /// Unlike `set()` which always passes `self` as the receiver, this
    /// propagates the original receiver through the prototype chain
    /// for accessor setters and property creation.
    ...
    /// [[Delete]] returning the boolean result without throwing.
    ///
    /// Unlike `delete_property_or_throw` this returns `Ok(false)` for
    /// non-configurable properties instead of throwing a TypeError.
    ...
    /// [[DefineOwnProperty]] returning the boolean result without throwing.
    ///
    /// Unlike `define_property_or_throw` this returns `Ok(false)` when
    /// the property is non-configurable instead of throwing a TypeError.
...
// ── Builder for exotic InternalObjectMethods ──

/// Builder for constructing custom [`InternalObjectMethods`] vtables from
/// outside the `boa_engine` crate.
///
/// This is the approved way to define exotic objects.  Start from the
/// ordinary methods (via [`InternalObjectMethods::ordinary`]) and override
/// specific internal methods via the builder methods.
///
/// # Example
///
/// ```rust
/// use boa_engine::object::{InternalObjectMethods, InternalMethodsBuilder};
///
/// fn my_get_prototype_of(
///     _obj: &JsObject,
...
///     .get_prototype_of(my_get_prototype_of)
///     .build();
/// ```
#[derive(Debug, Clone, Copy)]
pub struct InternalMethodsBuilder {
    methods: InternalObjectMethods,
}

impl InternalObjectMethods {
    /// Start building from the ordinary internal methods.
    pub const fn ordinary() -> InternalMethodsBuilder {
        InternalMethodsBuilder {
            methods: ORDINARY_INTERNAL_METHODS,
        }
    }
}

impl InternalMethodsBuilder {
    /// Override the [[GetPrototypeOf]] internal method.
    pub const fn get_prototype_of(
        mut self,
        f: fn(&JsObject, &mut Context) -> JsResult<super::JsPrototype>,
    ) -> Self {
        self.methods.__get_prototype_of__ = f;
        self
    }

    /// Override the [[SetPrototypeOf]] internal method.
    pub const fn set_prototype_of(
        mut self,
        f: fn(&JsObject, super::JsPrototype, &mut Context) -> JsResult<bool>,
    ) -> Self {
        self.methods.__set_prototype_of__ = f;
        self
    }

    /// Override the [[IsExtensible]] internal method.
    pub const fn is_extensible(
        mut self,
        f: fn(&JsObject, &mut Context) -> JsResult<bool>,
    ) -> Self {
        self.methods.__is_extensible__ = f;
        self
    }

    /// Override the [[PreventExtensions]] internal method.
    pub const fn prevent_extensions(
        mut self,
        f: fn(&JsObject, &mut Context) -> JsResult<bool>,
    ) -> Self {
        self.methods.__prevent_extensions__ = f;
        self
    }

    /// Override the [[GetOwnProperty]] internal method.
    pub const fn get_own_property(
        mut self,
        f: fn(
            &JsObject,
            &PropertyKey,
        ...
        self.methods.__get_own_property__ = f;
        self
    }

    /// Override the [[DefineOwnProperty]] internal method.
    pub const fn define_own_property(
        mut self,
        f: fn(
            &JsObject,
            &PropertyKey,
        ...
        self.methods.__define_own_property__ = f;
        self
    }

    /// Override the [[HasProperty]] internal method.
    pub const fn has_property(
        ...
        self.methods.__has_property__ = f;
        self
    }

    /// Override the [[Get]] internal method.
    pub const fn get(
        mut self,
        f: fn(
            &JsObject,
        ...
        self.methods.__get__ = f;
        self
    }

    /// Override the [[Set]] internal method.
    pub const fn set(
        mut self,
        f: fn(
            &JsObject,
        ...
        self.methods.__set__ = f;
        self
    }

    /// Override the [[Delete]] internal method.
    pub const fn delete(
        mut self,
        f: fn(
            &JsObject,
        ...
        self.methods.__delete__ = f;
        self
    }

    /// Override the [[OwnPropertyKeys]] internal method.
    pub const fn own_property_keys(
        mut self,
        f: fn(&JsObject, &mut Context) -> JsResult<Vec<PropertyKey>>,
    ) -> Self {
        self.methods.__own_property_keys__ = f;
        self
    }

    /// Finalise the builder and return the [`InternalObjectMethods`].
    pub const fn build(self) -> InternalObjectMethods {
        self.methods
    }
}
