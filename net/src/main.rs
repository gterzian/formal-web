...
pub fn run_net_process_v2(token: String) -> Result<(), String> {
    ipc::run_extension::<Request, Response>(&token, run_net_process_with_server)
}

/// Run the net extension over an already-established connection: the entry
/// point for the in-process transport, where the embedding process
/// registers this function as the net extension's runner instead of
/// launching the `formal-web-net` binary.
pub fn run_net_process_with_server(
