...
The modularity is goal orientated: it can be used to either support shipping on a platform with specific constraints (like using extensions for [BrowserEngineKit](https://developer.apple.com/documentation/browserenginekit)), or support 
platform specific performance (like integrating with Core Animation based compositing on Mac), for flexibility (like choosing a JS engine), or simply to reduce binary size and build time by re-using what is already on the system.
...
# backend and zero-copy IOSurface graphics. IPC is modular: Rust
# multiprocess over ipc-channel is the default; an extension-based
# BrowserEngineKit backend is planned (the XPC backend is not functional).
cargo build --release
cargo run --release
```
...
```bash
# V8 is the default and needs no feature flags; the others replace it:
