...
impl JsTypes for JscTypes {
    ...
    // ── Downcasts ────────────────────────────────────────────────────
    fn value_as_object(_v: &Self::JsValue) -> Option<Self::JsObject> { None }
...
impl JscEngine {
    ...
    fn property_key_to_jsstring(&self, key: &JscPropertyKey) -> Option<JscString> {
        match key { JscPropertyKey::String(s) => Some(s.clone()), JscPropertyKey::Symbol(_) => None }
    }
}
...
impl JsEngine<JscTypes> for JscEngine {
    ...
    fn set_realm_global_object(&mut self, _realm: &JscRealm, _global: JscObject, _this_value: Option<JscObject>)
        where JscTypes: JsTypesWithRealm { todo!("JSC set_realm_global_object") }
    fn set_default_global_bindings(&mut self, _realm: &JscRealm) -> Completion<(), JscTypes>
        where JscTypes: JsTypesWithRealm { Ok(()) }

    // ── §10.3 Built-in Function Objects ──────────────────────────────────
    fn create_builtin_function(
        &mut self,
        _behaviour: Box<dyn Fn(&[JscValue], JscValue, &mut dyn ExecutionContext<JscTypes>) -> Completion<JscValue, JscTypes>>,
        _length: u32,
        _name: JscPropertyKey,
        _realm: &JscRealm,
    ) -> JscFunction where JscTypes: JsTypesWithRealm {
        todo!("JSC CreateBuiltinFunction")
    }

    // ── §16 Script ────────────────────────────────────────────────────────
    fn evaluate_script(&mut self, source: &str, _realm: &JscRealm) -> Completion<JscValue, JscTypes>
        where JscTypes: JsTypesWithRealm
    {
    ...
    fn evaluate_module(&mut self, _source: &str, _realm: &JscRealm) -> Completion<JscObject, JscTypes>
        where JscTypes: JsTypesWithRealm { todo!("JSC module evaluation not available via C API") }

    // ── §25 ArrayBuffer — creation ─────────────────────────────────────
    fn allocate_array_buffer(&mut self, _constructor: JscConstructor, byte_length: u64, _max_byte_length: Option<u64>) -> Completion<JscArrayBuffer, JscTypes> {
        let len = byte_length as usize;
        let mut buf = vec![0u8; len].into_boxed_slice();
        let ptr = buf.as_mut_ptr() as *mut std::ffi::c_void;
        std::mem::forget(buf);
        let mut exception: *mut JSValueRef = std::ptr::null_mut();
        let raw = unsafe { JSObjectMakeArrayBufferWithBytesNoCopy(self.context.as_context_ref(), ptr, len, std::ptr::null_mut(), &mut exception) };
        if !exception.is_null() { return Err(JscValue { raw: exception }); }
        Ok(JscObject { raw, _phantom: std::marker::PhantomData })
    }
    fn detach_array_buffer(&mut self, _array_buffer: JscArrayBuffer, _key: Option<JscValue>) -> Completion<(), JscTypes> { Ok(()) }
    fn clone_array_buffer(&mut self, src: JscArrayBuffer, src_byte_offset: u64, src_length: u64, _clone_constructor: JscConstructor) -> Completion<JscArrayBuffer, JscTypes> {
        let global = self.context.global_object();
        let src_key = JscString::from_rust("__formal_web_clone_src");
        let mut exc: *mut JSValueRef = std::ptr::null_mut();
        unsafe { JSObjectSetProperty(self.context.as_context_ref(), global.raw, src_key.raw, src.as_value_ref(), kJSPropertyAttributeNone, &mut exc) };
        if !exc.is_null() { return Err(JscValue { raw: exc }); }
        let script_str = format!("new Uint8Array(__formal_web_clone_src).slice({}, {}).buffer", src_byte_offset, src_length);
        let script = JscString::from_rust(&script_str);
        let mut exc: *mut JSValueRef = std::ptr::null_mut();
        let result = unsafe { JSEvaluateScript(self.context.as_context_ref(), script.raw, std::ptr::null_mut(), std::ptr::null_mut(), 1, &mut exc) };
        let mut exc2: *mut JSValueRef = std::ptr::null_mut();
        unsafe { JSObjectDeleteProperty(self.context.as_context_ref(), global.raw, src_key.raw, &mut exc2) };
        if !exc.is_null() { return Err(JscValue { raw: exc }); }
        if result.is_null() { Err(JscUndefined::get(&self.context)) } else { Ok(JscObject { raw: result as *mut JSObjectRef, _phantom: std::marker::PhantomData }) }
    }
    fn allocate_shared_array_buffer(&mut self, _constructor: JscConstructor, byte_length: u64) -> Completion<JscSharedArrayBuffer, JscTypes> {
        let script_str = format!("new SharedArrayBuffer({})", byte_length);
        let script = JscString::from_rust(&script_str);
        let mut exc: *mut JSValueRef = std::ptr::null_mut();
        let result = unsafe { JSEvaluateScript(self.context.as_context_ref(), script.raw, std::ptr::null_mut(), std::ptr::null_mut(), 1, &mut exc) };
        if !exc.is_null() { return Err(JscValue { raw: exc }); }
        Ok(JscObject { raw: result as *mut JSObjectRef, _phantom: std::marker::PhantomData })
    }
...
impl ExecutionContext<JscTypes> for JscEngine {
    // ── §7.1 Type Conversion ──────────────────────────────────────────────
    fn to_primitive(&mut self, input: JscValue, _preferred_type: Option<PreferredType>) -> Completion<JscValue, JscTypes> { Ok(input) }
    fn to_boolean(&self, value: &JscValue) -> bool { unsafe { JSValueToBoolean(self.context.as_context_ref(), value.raw) } }
    ...
    fn to_numeric(&mut self, value: JscValue) -> Completion<Numeric<JscTypes>, JscTypes> {
        ...
        match js_type { JSType::kJSTypeBigInt => Ok(Numeric::BigInt(JscBigInt { value })), _ => self.to_number(value).map(Numeric::Number) }
    }
    ...
    fn to_uint8_clamp(&mut self, value: JscValue) -> Completion<u8, JscTypes> {
        self.to_number(value).map(|n| if n <= 0.0 { 0 } else if n >= 255.0 { 255 } else { (n + 0.5).floor() as u8 })
    }
    ...
    fn to_object(&mut self, value: JscValue) -> Completion<JscObject, JscTypes> {
        let js_type = unsafe { JSValueGetType(self.context.as_context_ref(), value.raw) };
        match js_type {
            JSType::kJSTypeObject => Ok(JscObject { raw: value.raw as *mut JSObjectRef, _phantom: PhantomData }),
            JSType::kJSTypeUndefined | JSType::kJSTypeNull => {
                let message = JscString::from_rust("Cannot convert undefined or null to object");
                Err(JscValue { raw: unsafe { JSValueMakeString(self.context.as_context_ref(), message.raw) } })
            }
            _ => Ok(JscObject { raw: value.raw as *mut JSObjectRef, _phantom: PhantomData }),
        }
    }
    fn to_property_key(&mut self, value: JscValue) -> Completion<JscPropertyKey, JscTypes> {
        let js_type = unsafe { JSValueGetType(self.context.as_context_ref(), value.raw) };
        if js_type == JSType::kJSTypeSymbol {
            return Ok(JscPropertyKey::Symbol(unsafe { JscSymbol::from_value(value) }));
        }
        let mut exception: *mut JSValueRef = std::ptr::null_mut();
        let raw = unsafe { JSValueToStringCopy(self.context.as_context_ref(), value.raw, &mut exception) };
        if !exception.is_null() { return Err(JscValue { raw: exception }); }
    ...
    fn to_length(&mut self, value: JscValue) -> Completion<u64, JscTypes> { self.to_number(value).map(|n| if n <= 0.0 { 0 } else { n.min(f64::from(u32::MAX)) as u64 }) }
    fn canonical_numeric_index_string(&self, argument: &JscString) -> Option<f64> {
        let s = argument.to_rust();
        if let Ok(n) = s.parse::<f64>() {
            if n.to_string() == s || (n.is_infinite() && (s.starts_with('-') || s.starts_with('+'))) { return Some(n); }
        }
        None
    }
    ...
    fn is_property_key(&self, value: &JscValue) -> bool {
        match unsafe { JSValueGetType(self.context.as_context_ref(), value.raw) } {
            JSType::kJSTypeString | JSType::kJSTypeSymbol => true, _ => false
        }
    }
    fn same_value(&self, x: &JscValue, y: &JscValue) -> bool { unsafe { JSValueIsStrictEqual(self.context.as_context_ref(), x.raw, y.raw) } }
    fn same_value_zero(&self, x: &JscValue, y: &JscValue) -> bool { unsafe { JSValueIsStrictEqual(self.context.as_context_ref(), x.raw, y.raw) } }
    ...
    fn is_strictly_equal(&self, x: &JscValue, y: &JscValue) -> bool { unsafe { JSValueIsStrictEqual(self.context.as_context_ref(), x.raw, y.raw) } }
    ...
    // ── §7.4 Iteration ───────────────────────────────────────────────────
    fn get_iterator(&mut self, object: JscValue, _kind: IteratorKind, _method: Option<JscFunction>) -> Completion<IteratorRecord<JscTypes>, JscTypes> {
        let global = self.context.global_object();
        let mut exc: *mut JSValueRef = std::ptr::null_mut();
        let symbol_str = JscString::from_rust("Symbol");
        let symbol_val = unsafe { JSObjectCopyProperty(self.context.as_context_ref(), global.raw, symbol_str.raw, &mut exc) };
        if !exc.is_null() { return Err(JscValue { raw: exc }); }
        let symbol_obj = JscObject { raw: symbol_val as *mut JSObjectRef, _phantom: PhantomData };
        let iter_str = JscString::from_rust("iterator");
        let iterator_sym = unsafe { JSObjectCopyProperty(self.context.as_context_ref(), symbol_obj.raw, iter_str.raw, &mut exc) };
        if !exc.is_null() { return Err(JscValue { raw: exc }); }
        let method = self.get_method(object, JscPropertyKey::Symbol(unsafe { JscSymbol::from_value(JscValue { raw: iterator_sym }) }))?;
        ...
        let mut exc: *mut JSValueRef = std::ptr::null_mut();
        let next_val = unsafe { JSObjectCopyProperty(self.context.as_context_ref(), iter_obj, next_str.raw, &mut exc) };
        if !exc.is_null() { return Err(JscValue { raw: exc }); }
        let next_method = JscObject { raw: next_val as *mut JSObjectRef, _phantom: PhantomData };
        Ok(IteratorRecord { iterator: JscObject { raw: iter_obj, _phantom: PhantomData }, next_method, done: false })
    }
    fn iterator_step_value(&mut self, iterator: &mut IteratorRecord<JscTypes>) -> Completion<Option<JscValue>, JscTypes> {
        let iter_val = JscValue { raw: iterator.iterator.raw as *mut JSValueRef };
        ...
        let mut exc: *mut JSValueRef = std::ptr::null_mut();
        let done_val = unsafe { JSObjectCopyProperty(self.context.as_context_ref(), result_obj, done_str.raw, &mut exc) };
        if !exc.is_null() { iterator.done = true; return Err(JscValue { raw: exc }); }
        let done = unsafe { JSValueToBoolean(self.context.as_context_ref(), done_val) };
        if done { iterator.done = true; return Ok(None); }
        let value_str = JscString::from_rust("value");
        let mut exc: *mut JSValueRef = std::ptr::null_mut();
        let value = unsafe { JSObjectCopyProperty(self.context.as_context_ref(), result_obj, value_str.raw, &mut exc) };
        if !exc.is_null() { iterator.done = true; return Err(JscValue { raw: exc }); }
        Ok(Some(JscValue { raw: value }))
    }
    fn iterator_close(&mut self, iterator: IteratorRecord<JscTypes>, completion: Completion<JscValue, JscTypes>) -> Completion<JscValue, JscTypes> {
        let return_str = JscString::from_rust("return");
        let return_key = JscPropertyKey::String(return_str);
        let inner_result = self.get_method(JscValue { raw: iterator.iterator.raw as *mut JSValueRef }, return_key);
        match inner_result {
            Ok(Some(return_fn)) => {
                let iter_val = JscValue { raw: iterator.iterator.raw as *mut JSValueRef };
                match EcmascriptHost::call(self, &return_fn, &iter_val, &[]) { Ok(_) => completion, Err(e) => { if completion.is_err() { return completion; } Err(e) } }
            }
            Ok(None) => completion,
            Err(e) => { if completion.is_err() { return completion; } Err(e) }
        }
    }
    fn async_iterator_close(&mut self, iterator: IteratorRecord<JscTypes>, completion: Completion<JscValue, JscTypes>) -> Completion<JscValue, JscTypes> { self.iterator_close(iterator, completion) }

    // ── §9.3 Realm — runtime access ──────────────────────────────────────
    fn current_realm(&self) -> JscRealm where JscTypes: JsTypesWithRealm { JscRealm { raw: self.context.raw } }
    fn realm_intrinsics(&self, _realm: &JscRealm) -> RealmIntrinsics<JscTypes> where JscTypes: JsTypesWithRealm { todo!("JSC realm_intrinsics") }
    ...
    // ── Error Reporting ──────────────────────────────────────────────────
    fn report_error(&mut self, message: &str) { log::error!("unhandled exception: {message}"); }
}
...
impl EcmascriptHost<JscTypes> for JscEngine {
    ...
    fn call(&mut self, callable: &JscObject, this_arg: &JscValue, args: &[JscValue]) -> Completion<JscValue, JscTypes> {
        let this_obj = if unsafe { JSValueGetType(self.context.as_context_ref(), this_arg.raw) } == JSType::kJSTypeObject { this_arg.raw as *mut JSObjectRef } else { std::ptr::null_mut() };
        let args_raw: Vec<*mut JSValueRef> = args.iter().map(|v| v.raw).collect();
        let mut exception: *mut JSValueRef = std::ptr::null_mut();
        let result = unsafe { JSObjectCallAsFunction(self.context.as_context_ref(), callable.raw, this_obj, args_raw.len(), args_raw.as_ptr(), &mut exception) };
        if !exception.is_null() { return Err(JscValue { raw: exception }); }
        Ok(JscValue { raw: result })
    }
    fn perform_a_microtask_checkpoint(&mut self) -> Completion<(), JscTypes> { Ok(()) }
    fn report_exception(&mut self, error: JscValue) { log::error!("uncaught callback error"); let _ = error; }

    fn value_undefined(&mut self) -> JscValue { JscValue { raw: unsafe { JSValueMakeUndefined(self.context.as_context_ref()) } } }
    fn value_null(&mut self) -> JscValue { JscValue { raw: unsafe { JSValueMakeNull(self.context.as_context_ref()) } } }
