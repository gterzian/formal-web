...
A wrapper and its Rust platform object form the cycle `wrapper → platform data →
reflector → wrapper`.  The platform wrapper is pinned by the realm-lifetime
managed root (`create_object_with_any`), so JavaScriptCore never collects the
pair; wrappers are released at engine teardown.  V8 collects such cycles
because platform objects and their edges live in the cppgc unified heap, and
Boa because `boa_gc` traces `#[gc_struct]` fields.  JSC has no equivalent for
Rust objects: its only host-object edge API is the ObjC managed-reference
mechanism (`JSManagedValue` + `addManagedReference:withOwner:`), which reports
an edge to the collector but does not let the collector enumerate a Rust
object's references.

The intended model is a per-object owner: each platform object's holder (see
`create_object_with_any`) owns the managed values for the JS values its Rust
data holds, and those edges are torn down when the holder deallocs, so cycles
become collectable.  `JscGcContext::create_platform_object` and
`JscManagedValue::new_owned(Owner::PlatformObject)` provide the plumbing;
registering the edges needs a `Trace`-like walk of each platform object's Rust
data (a `GcTraceable`/`GcOwner`/`GcCellSet` design, with the walk generated by
`#[gc_struct]`).

**Dead end:** that model was implemented on `origin/jsc_objc_2` (`GcCell<T>` =
`Rc<RefCell<T>>` plus managed-reference edges, with the per-object owner
exported on the reflector).  Registering the managed edges and exporting the
per-object owner each SIGSEGV system JavaScriptCore's GC during the heavy
streams tests (`tee.any.js`), so both are gated off there and the design is not
on this line.  No public JSC API traces host-object references; the
managed-reference API is the only candidate and it crashes under this load.

Until that changes, `GcCell` remains `Rc<RefCell<T>>` and JS values held by Rust
that the JS graph cannot reach are rooted explicitly (`Callback`, for example,
via `create_root`), which is what keeps callbacks alive across GC but keeps the
cycle uncollectable.
