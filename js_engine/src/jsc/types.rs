...
impl JscContext {
    ...
    pub fn global_object(&self) -> JscObject {
        ...
        assert!(!raw.is_null());
        JscObject { raw, _phantom: PhantomData }
    }
}

impl Default for JscContext { fn default() -> Self { Self::new() } }

impl Drop for JscContext {
    fn drop(&mut self) { unsafe { JSGlobalContextRelease(self.raw) } }
}
...
impl JscString {
    ...
    pub unsafe fn from_raw(raw: *mut JSStringRef) -> Self { Self { raw } }

    pub fn as_raw(&self) -> *mut JSStringRef { self.raw }

    pub fn to_rust(&self) -> String {
        let max_size = unsafe { JSStringGetMaximumUTF8CStringSize(self.raw) };
        let mut buffer = vec![0u8; max_size];
        let count = unsafe { JSStringGetUTF8CString(self.raw, buffer.as_mut_ptr() as *mut c_char, max_size) };
        buffer.truncate(count.saturating_sub(1));
        String::from_utf8_lossy(&buffer).to_string()
    }

    pub fn len(&self) -> usize { unsafe { JSStringGetLength(self.raw) } }
    pub fn is_empty(&self) -> bool { self.len() == 0 }
}

impl Clone for JscString {
    fn clone(&self) -> Self { Self { raw: unsafe { JSStringRetain(self.raw) } } }
}

impl Drop for JscString { fn drop(&mut self) { unsafe { JSStringRelease(self.raw) } } }
...
impl PartialEq for JscString {
    fn eq(&self, other: &Self) -> bool { unsafe { JSStringIsEqual(self.raw, other.raw) } }
}
impl Eq for JscString {}

impl std::hash::Hash for JscString {
    fn hash<H: std::hash::Hasher>(&self, state: &mut H) { self.to_rust().hash(state); }
}
...
impl JscValue {
    ...
    pub unsafe fn from_raw(raw: *mut JSValueRef) -> Self { Self { raw } }
    pub fn as_raw(&self) -> *mut JSValueRef { self.raw }
...
impl JscUndefined {
    pub fn get(ctx: &JscContext) -> JscValue {
        JscValue { raw: unsafe { JSValueMakeUndefined(ctx.as_context_ref()) } }
    }
}
...
impl JscNull {
    pub fn get(ctx: &JscContext) -> JscValue {
        JscValue { raw: unsafe { JSValueMakeNull(ctx.as_context_ref()) } }
    }
}
...
impl JscObject {
    ...
    pub unsafe fn from_raw(raw: *mut JSObjectRef) -> Self { Self { raw, _phantom: PhantomData } }
    pub fn as_raw(&self) -> *mut JSObjectRef { self.raw }
    pub fn as_value_ref(&self) -> *mut JSValueRef { self.raw as *mut JSValueRef }
    pub fn as_value(&self) -> JscValue { JscValue { raw: self.as_value_ref() } }
...
impl JscSymbol {
    ...
    pub unsafe fn from_value(value: JscValue) -> Self { Self { value } }
    pub fn as_value(&self) -> &JscValue { &self.value }
}
...
impl JscPropertyKey {
    pub fn from_rust(s: &str) -> Self { JscPropertyKey::String(JscString::from_rust(s)) }
}
...
impl JscRealm {
    ...
    pub unsafe fn from_raw(raw: *mut JSGlobalContextRef) -> Self { Self { raw } }
    pub fn as_raw(&self) -> *mut JSGlobalContextRef { self.raw }
}
