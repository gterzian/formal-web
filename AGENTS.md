...
- Startup should use an explicit runtime-message handshake: on `resumed`, Rust sends `FreshTopLevelTraversable` to Lean; Lean creates and bootstraps a fresh top-level traversable, stubs the initial fetch/navigation to completion, and only after the traversable has an active document with no ongoing navigation should it send `NewTopLevelTraversable` back to Rust; only that Rust-side message handler should call `request_redraw()` and then `user_agent_note_rendering_opportunity`.
