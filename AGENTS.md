...
- Route supported winit input events through the Rust-side Blitz `UiEvent` translation first, then send a `DispatchEvent|...` user-agent task message to Lean, then apply the same `UiEvent` to the current `BaseDocument` locally.
