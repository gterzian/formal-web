...
- Build dedicated `formal-web-content` and `formal-web-net` executables from the top-level package and launch those sidecars directly from the user agent instead of respawning the main `formal-web` executable with role flags.
- When a runner or tool launches sibling sidecars from `target/{debug,release}`, build the top-level package with `cargo build --bins` for that profile so the spawned executables stay in sync with the main binary.
