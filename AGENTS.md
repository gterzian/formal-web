...
- Keep paint scheduling as a user-agent decision: after update-the-rendering completes, the user agent should queue a paint request back onto the owning event loop using the document id, and the event loop should resolve that id through its local Rust-document map before calling the host paint FFI.
