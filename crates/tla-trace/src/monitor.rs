...
impl Monitor {
    pub fn new(output_dir: impl Into<PathBuf>, receiver: IpcReceiver<LogEntry>) -> Self {
        Self {
    ...
    fn writer_for_spec(&mut self, spec: &str) -> Result<&mut BufWriter<File>, String> {
        if !self.writers.contains_key(spec) {
            let file_path = trace_file_path(&self.output_dir, spec);
        ...
        self.writers
            .get_mut(spec)
            .ok_or_else(|| format!("missing trace writer for spec {spec}"))
    }
...
fn trace_file_path(output_dir: &Path, spec: &str) -> PathBuf {
