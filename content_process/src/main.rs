...
impl ContentRuntime {
    ...
    fn set_viewport(&mut self, viewport: ViewportSnapshot) {
        ...
        for document in self.documents.values_mut() {
            document.document.set_viewport(runtime_viewport.clone());
        }
    }

    fn create_empty_document(&mut self, document_id: u64) {
        let document = HtmlDocument::from_html(EMPTY_HTML_DOCUMENT, self.document_config(None));
    ...
    fn create_loaded_document(&mut self, document_id: u64, url: String, body: String) {
        let document =
            HtmlDocument::from_html(&body, self.document_config(Some(url.clone())));
        self.documents.insert(
            document_id,
            ContentDocument { document },
        );
    ...
    fn dispatch_event(&mut self, document_id: u64, event: String) -> Result<(), String> {
        ...
        BlitzDocument::handle_ui_event(&mut document.document, event);
        Ok(())
    }
    ...
    fn update_the_rendering(&mut self, document_id: u64) -> Result<(), String> {
        ...
        for _ in 0..3 {
            document.document.resolve(animation_time);
            if !document.document.has_pending_critical_resources() {
                break;
            }
        }

        let viewport = document.document.viewport().clone();
        ...
        let viewport_scroll = document.document.viewport_scroll();
        self.event_sender
